...
use crate::kv::{Database, DatabaseSettings, DatabaseSource, DbHash};
	...
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{
	...
	#[cfg(not(test))]
	match crate::kv::upgrade::upgrade_db::<Block, C>(client, path, _source) {
		Ok(_) => (),
		Err(_) => return Err("Frontier DB upgrade error".to_string()),
	}

	let mut db_config = kvdb_rocksdb::DatabaseConfig::with_columns(crate::kv::columns::NUM_COLUMNS);
	...
	#[cfg(not(test))]
	crate::kv::upgrade::update_version(path).map_err(|_| "Cannot update db version".to_string())?;
	...
	C: sp_blockchain::HeaderBackend<Block> + Send + Sync,
{
	...
	let mut config = parity_db::Options::with_columns(path, crate::kv::columns::NUM_COLUMNS as u8);
	config.columns[crate::kv::columns::BLOCK_MAPPING as usize].btree_index = true;

	let db = parity_db::Db::open_or_create(&config).map_err(|err| format!("{}", err))?;
	// write database version only after the database is succesfully opened
	#[cfg(not(test))]
	crate::kv::upgrade::update_version(path).map_err(|_| "Cannot update db version".to_string())?;
	Ok(Arc::new(crate::kv::parity_db_adapter::DbAdapter(db)))
}
